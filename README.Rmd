...
To process new layers, there are **three** options:
1. vector 
2. vector to raster (rasterize)
3. raster 
...
1. Where **multiple metrics** are required (e.g. species number, species name) - processing should likely be done in _**vector**_ format.
2. For **area-only metrics** - _**raster**_ processing will be faster. The `01_load.R` script ensures the raster is in Hectares BC format prior to processing. 
